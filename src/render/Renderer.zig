...
pub fn drawRegion9Patch(
    self: *Renderer,
    src_outer: math.Rect(u8),
    src_inner: math.Rect(u8),
    dst: math.Rect(f32),
) !void {
    _ = src_inner; // TODO
    try self.drawRegion(src_outer, dst);
}
...
pub fn drawSprite(self: *Renderer, sprite: Spritesheet.Sprite, pos: math.Vec2(f32)) !void {
    ...
    try self.drawRegion(sprite_info.pixel_rect(), .{
        .x = pos.x,
        .y = pos.y,
...
pub fn drawSprite9Patch(
    ...
    return self.drawSpriteStretch(sprite, rect);
}
